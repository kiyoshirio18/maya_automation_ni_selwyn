...
if "filtered_xlsx" not in st.session_state:
    st.session_state.filtered_xlsx = None
...
st.header("Filter Status")

with st.form(key="dispute"):
...
if st.session_state.remark_df is not None:
    st.session_state.status_list = st.session_state.remark_df["Status"].unique().to_list()

    # Create columns for filters and status selection
    col1, col2 = st.columns(2, vertical_alignment="bottom")
    st.session_state.filter_status = col1.multiselect("Status", sorted(st.session_state.status_list), default=sorted(st.session_state.status_list))

    # Button for filtering the DataFrame
    if col2.button("Filter", use_container_width=True):
        if st.session_state.filter_status:
            # Apply the filter based on selected status
            filtered_df = st.session_state.remark_df.filter(pl.col("Status").is_in(st.session_state.filter_status))
            st.session_state.filtered_df = filtered_df
        else:
            # If no status is selected, show the full DataFrame
            st.session_state.filtered_df = st.session_state.remark_df

if st.session_state.filtered_df is not None and st.session_state.filtered_xlsx is None:
    st.session_state.filtered_xlsx = BytesIO()
    st.session_state.filtered_df.write_excel(
        st.session_state.filtered_xlsx,
        autofit = True
    )
    st.session_state.filtered_xlsx.seek(0)
    st.session_state.date = st.session_state.filtered_df["Date"].max()


if st.session_state.filtered_xlsx is not None:
    st.download_button(
        label = "Download Filtered Remark Report",
        data = st.session_state.filtered_xlsx.getvalue(),
        file_name = f"maya_filter_{st.session_state.date}.xlsx",
